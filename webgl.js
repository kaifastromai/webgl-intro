...
function createGeo() {
    ...
    for (let i = 0; i < slices; i++) {
        ...
        for (let stck_c = 0; stck_c < stacks; stck_c++) {
            ...
            vec3.add(lerp_vec, vec3.lerp(lerp_vec, ic, oc, prcnt), vec3.fromValues(rad * x, rad * y, 0));
...
function setColors() {
    var verts = render_array.length;
    var color_array = [];
    for (let i = 0; i < verts; i++) {
        var cf = 1; //(i / (verts));
        color_array.push(
        // left column front
        255 * cf, 255 * cf, 250, 255 * cf, 255 * cf, 250, 255 * cf, 255 * cf, 250, 120 * cf, 255 * cf, 255, 120 * cf, 255 * cf, 255);
    }
    gl.bufferData(gl.ARRAY_BUFFER, new Uint8Array(color_array), gl.STATIC_DRAW);
}
function setColorsLns() {
    var verts = disk_array.length;
    var color_array = [];
    for (let i = 0; i < verts; i++) {
        var cf = 1; //(i / (verts))
        color_array.push(
        // left column front
        255 * cf, 255 * cf, 255, 255 * cf, 255 * cf, 255, 255 * cf, 255 * cf, 255, 255 * cf, 255 * cf, 255, 255 * cf, 255 * cf, 255, 255 * cf, 255 * cf, 255);
    }
    gl.bufferData(gl.ARRAY_BUFFER, new Uint8Array(color_array), gl.STATIC_DRAW);
}
